...
sets = ['train', 'test', 'val']
classes = ['Aging', 'frayed','craked','Normal'] #要一模一樣
...
def convert_annotation(image_id):
    ...
    in_file = open('D:\TireCheck\Tire_dataset/annotation/%s.xml' % (image_id), encoding='utf-8')
    # 準備在對應的image_id 中寫入對應的label，分別為
    # <object-class> <x> <y> <width> <height>
    out_file = open('Tire_dataset\labels\%s.txt' % (image_id), 'w', encoding='utf-8')
    # 解析xml檔案
...
　　　　２．同時對所有的圖片檔案進行解析和轉化，將其對應的bundingbox 以及類別的資訊全部解析寫到label 檔案中去
    ...
    '''
    # 先找labels資料夾如果不存在則建立
    if not os.path.exists('D:\TireCheck\Tire_dataset\labels'):
        os.makedirs('D:\TireCheck\Tire_dataset\labels')
    # 讀取在ImageSets/Main 中的train、test..等檔案的內容
    # 包含對應的檔名稱
    image_ids = open('Tire_dataset/ImageSets/%s.txt' % (image_set)).read().strip().split()
    # 開啟對應的2012_train.txt 檔案對其進行寫入準備
    list_file = open('Tire_dataset/ImageSets/%s.txt' % (image_set), 'w')
    # 將對應的檔案_id以及全路徑寫進去並換行
    for image_id in image_ids:
        list_file.write('Tire_dataset/images/%s.jpg\n' % (image_id)) 
